...
public class BoardFileController {
    ...
    @PutMapping(BoardApi.ID_ONLY + "/files")
    public ResponseEntity<?> updateBoardFiles(
            @PathVariable Long boardId,
            // 쿼리 파라미터나 HTML 품 데이터를 Java 객체로 바인딩할 때 사용
            // @ModeAttribute가 가장 안정적인 방식
            @ModelAttribute BoardFileUpdateRequestDto dto) {
        boardFileService.uploadBoardFiles(boardId, (List<MultipartFile>) dto);
        return ResponseEntity.noContent().build();
    }
}
